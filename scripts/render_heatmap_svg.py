...
plays on load then freezes -- no looping), a Less->More legend, and a real
stats footer. Reads ONLY the local JSON written by fetch_contributions.py --
no network calls here, no third-party API.
"""
import datetime
import json
import os
...
""".strip()
    ...
    parts.append(f'<text x="{canvas_w/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
                 f'text-anchor="middle">rehbar@github: ~/contributions --graph</text>')
