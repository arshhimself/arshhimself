"""
Convert a portrait photo into a CLEAN, monochrome ASCII-art SVG (one
...
a busy background washes out to blank) reads as neat and legible.
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "rehbar-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">rehbar@github: ~$ ./portrait.sh</text>')
...
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'rehbar@github:~$ whoami <tspan fill="{INK}">Rehbar Khan</tspan></text>')
